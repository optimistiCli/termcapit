...
COMPILED_TMP="$(mktemp -p /tmp ${0##*/}_compiled.XXXXXXXXXX)" || brag_and_exit "Can not create temp file for compiling"
...
if [ -f "$SOURCE" ] ; then
	...
	JOINED_TMP="$(mktemp -p /tmp ${0##*/}_joined.XXXXXXXXXX)" || brag_and_exit "Can not create temp file for joining"
	add_to_cleanable "$JOINED_TMP"

	sed ':x; /\\$/ { N; s/\\\n[\t]*/@NEWLINE@/; tx }' < "$SOURCE" >> "$JOINED_TMP"
	...
	grep -vE "^($OVERLAP_NAMES)[:|]" < "$JOINED_TMP" | sed 's/@NEWLINE@/\\\n\t/g' >> "$COMPILED_TMP"
elif [ ! -e "$SOURCE" ] ; then
